...
function WordBank({ words, onSaveWords, onBack }) {
  const [wbTab, setWbTab] = useState("add"); // "add" | "list"
  ...
  // Builds the same lenient "Слово — перевод" text format used everywhere
  // else in the app, one word per line.
  const wordsAsText = () => words.map((w) => `${w.kz} — ${w.ru}`).join("\n");

  const handleCopyList = async () => {
    try {
      await navigator.clipboard.writeText(wordsAsText());
      setExportStatus({ type: "ok", text: "Список скопирован в буфер обмена" });
    } catch (e) {
      setExportStatus({ type: "error", text: "Не удалось скопировать — попробуй скачать файлом вместо этого" });
    }
  };

  const handleDownloadTxt = () => {
    try {
      const dataUri = "data:text/plain;charset=utf-8," + encodeURIComponent(wordsAsText());
      const link = document.createElement("a");
      link.href = dataUri;
      link.download = `slova-${new Date().toISOString().slice(0, 10)}.txt`;
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      setExportStatus({ type: "ok", text: "Файл сохраняется — смотри загрузки телефона" });
    } catch (e) {
      setExportStatus({ type: "error", text: "Не удалось сохранить файл" });
    }
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0 }}>
      ...
      <div style={{ display: "flex", gap: 8, padding: "16px 24px 0" }}>
        <button
          onClick={() => setWbTab("add")}
          style={{
            flex: 1,
            border: "none",
            borderRadius: 12,
            padding: "10px",
            background: wbTab === "add" ? COLORS.steppe : COLORS.cloud,
            color: wbTab === "add" ? "#fff" : "#7A8AA6",
            fontFamily: "'Baloo 2', sans-serif",
            fontWeight: 700,
            fontSize: 14,
            cursor: "pointer",
        ...
        <button
          onClick={() => setWbTab("list")}
          style={{
            flex: 1,
            border: "none",
            borderRadius: 12,
            padding: "10px",
            background: wbTab === "list" ? COLORS.steppe : COLORS.cloud,
            color: wbTab === "list" ? "#fff" : "#7A8AA6",
            fontFamily: "'Baloo 2', sans-serif",
            fontWeight: 700,
            fontSize: 14,
            cursor: "pointer",
      ...
      <div style={{ flex: 1, overflowY: "auto", padding: 24, display: "flex", flexDirection: "column", gap: 20, minHeight: 0 }}>
        ...
        {exportStatus && (
          <div style={{ color: exportStatus.type === "ok" ? COLORS.steppe : COLORS.coral, fontWeight: 700, fontSize: 13, textAlign: "center" }}>
            {exportStatus.text}
          </div>
        )}
      ...
      >
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <button
            ...
            style={{
              background: wordCount === 0 ? "#EDEFF3" : `linear-gradient(135deg, ${COLORS.sun} 0%, #FFB020 100%)`,
              border: "none",
              borderRadius: 18,
              padding: "16px 14px",
              display: "flex",
          ...
          >
            ...
            <div style={{ fontSize: 12, opacity: 0.75 }}>
              {wordCount === 0
                ? "сначала добавь слова"
                : newWordsCount > 0
                ? `${newWordsCount} ждут изучения`
                : "всё изучено — жми ещё раз"}
            </div>
          </button>
...
function LearnSession({ words, mode = "new", onRecordResult, onFinish }) {
  ...
  if (phase === "quiz") {
    return (
      <div style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0 }}>
        ...
        <div style={{ flex: 1, overflowY: "auto", minHeight: 0, padding: 24, display: "flex", flexDirection: "column", gap: 20 }}>
          <div
            style={{
              background: COLORS.cloud,
              borderRadius: 20,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", minHeight: 0 }}>
      ...
      <div style={{ flex: 1, overflowY: "auto", minHeight: 0, padding: 24, display: "flex", flexDirection: "column", justifyContent: "center", gap: 24 }}>
        <div
