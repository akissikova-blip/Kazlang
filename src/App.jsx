...
// Voluntary review queue for when everything is already mastered (no new
// words, no mistakes to fix) but the child still wants to practice. Prefers
// already-studied words (so it's genuine repetition), falling back to
// whatever exists if nothing has been studied yet at all.
        ...
        >
          ...
          {stage === "home" && profile && (
            <HomeScreen
              ...
              hasStudiedWords={words.some((w) => (w.stats?.correct || 0) + (w.stats?.wrong || 0) > 0)}
            />
          )}
          {stage === "words" && (
            <WordBank
...
function LearnSession({ words, onRecordResult, onFinish }) {
  const [queue, setQueue] = useState(() => getLearningQueue(words, 10));
...
function WordBuilderGame({ words, onRecordResult, onFinish }) {
  ...
  const [queue, setQueue] = useState(() => getLearningQueue(eligibleWords, 10));
  const [index, setIndex] = useState(0);
